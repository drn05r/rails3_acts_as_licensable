class AddRails3ActsAsLicensableLicenses < ActiveRecord::Migration
  def self.up
    ...
    @license_config['license_options'].each do |lo_id, license_option|
      license_option_recs[lo_id] = Rails3ActsAsLicensable::LicenseOption.create(license_option)
      license_option_recs[lo_id].save!
    end
    @license_config['licenses'].each do |license_id, license|
